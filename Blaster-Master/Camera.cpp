...
void Camera::UpdateCamera()
{
	...
	RECT bouncing;
	bouncing.left = (long) (cam_x + screen_width / 2 - max_width);
	bouncing.right = (long) (cam_x + screen_width / 2 + max_width);
	bouncing.top = (long) (cam_y - screen_height / 2 + max_height);
	bouncing.bottom = (long) (cam_y - screen_height / 2 - max_height);
	...
	if (player_x < bouncing.left) // move left
	{
		...
		if (cam_x < bouncingMap.left) cam_x = (float) bouncingMap.left;
	}
	...
	if (player_y > bouncing.top)
	{
		...
		if (cam_y > bouncingMap.top) cam_y = (float) bouncingMap.top;
	}
